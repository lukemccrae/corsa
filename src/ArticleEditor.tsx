...
export const ArticleEditor = (props: ArticleEditorProps) => {
  ...
  const addItem = (arg: string) => {
    ...
    switch (arg) {
      ...
      case 'paceTable':
        newElement = {
          paceTable: {
            columns: ['Mile', 'Gain', 'Elapsed', 'Profile', 'Pace', 'GAP'],
            miles: [-1, 4],
          },
          editing: false,
          id,
  ...
  const isPaceTable = (
    e: ArticleElement
  ): e is { paceTable: PaceTableType; editing: boolean; id: string } => 'paceTable' in e;
      ...
      return (
        <Box sx={{ display: 'flex', flexDirection: 'row' }}>
          <PaceTableEditor
            ...
            updatePaceTableElement={updatePaceTableElement}
            element={element.paceTable}
            mileData={props.mileData}
            lastMileDistance={props.lastMileDistance}
            index={index}
            elements={{}}
            createNewElementsMap={props.createNewElementsMap}
            setElementIdsForOrder={props.setElementIdsForOrder}
            elementIdsForOrder={props.elementIdsForOrder}
          ></PaceTableEditor>
        </Box>
      );
  ...
  return (
    <>
      {props.elements && (
        <Stack spacing={2}>
          {props.elementIdsForOrder?.map((id, index) => {
            return (
              <Card key={id} sx={{ padding: 3, display: 'flex' }}>
                <CardContent>
                  <Box
                    sx={{
                      display: 'flex',
      ...
      <Dialog open={dialogOpen} onClose={closeDialog}>
        ...
        <DialogContent>
          <Stack spacing={2}>
            <Button variant="contained" onClick={() => addItem('text')}>
              Create Article
            </Button>
            <Button variant="contained" onClick={() => addItem('paceTable')}>
              Create Pace Table
            </Button>
          </Stack>
        </DialogContent>
