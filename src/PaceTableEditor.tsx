...
export const PaceTableEditor = (props: PaceTableEditorProps) => {
  ...
  const handleSave = () => {
    if (miles.length !== 2 || columns.length === 0) {
      alert(
        'Please ensure you have both start and end miles set, and at least one column selected.'
      );
      return;
    }
    // Validate mile range
  ...
  const handleConfirmDelete = () => {
    props.createNewElementsMap(
      [...Object.values(props.elements)].filter((_, i) => i !== props.index)
    );
  };
  ...
  return (
    <Box sx={{ display: 'flex', flexDirection: 'column' }}>
      ...
      <Box sx={{ display: 'flex', flexDirection: 'row' }}>
        <PaceTable cols={columns} miles={miles} plan={plan}></PaceTable>
