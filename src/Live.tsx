...
export const Live = () => {
  ...
  useEffect(() => {
    ...
    const subscribe2Waypoints = client.graphql({ query: subscriptions.subscribe2Waypoints }).subscribe({
      next: ({ data }) => {
        const { lat, long } = data.subscribe2Waypoints;
        
        // Parse the lat and long values from strings to floats
        ...
        if (!isNaN(parsedLat) && !isNaN(parsedLong)) {
          ...
          console.warn('Invalid latitude or longitude:', lat, long);
        }
      },
